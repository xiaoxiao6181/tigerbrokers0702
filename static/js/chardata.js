...
$(".amount-view").on('click','.amount-box',function(){
	...
	if (my_money < order_price) {
		console.log(my_money,order_price)
...
function toggle_order_confirm_panel(type) {
    if (type == 'lookup') {
        var typename = I18n.t('product.buyRise');
        order_type = 0;

        $('.order_type').removeClass('fall');
        $('.order_type').addClass('rise');
    } else {
        var typename = I18n.t('product.buyFall');
        order_type = 1;
...
function addorder() {
	...
	AjaxUtil.post(posturl+postdata).then(
		function(resdata) {
		...
		function(err) {
			const message = (err && (err.info || err.msg)) || '下单失败，请稍后重试';
			err_info(message);
		}
	)
...
function goorder() {
	...
	// get_this_order();
	// AjaxUtil.post(posturl+postdata).then(res=>{
	// 	console.log(res)
	// 	get_this_order();
	// })
